...
namespace JoculMeu
{
	...
	void MainMenuState::Init()
	{
		...
		this->_data->assets.LoadTexture("Info Button", INFO_BUTTON);
		...
		this->_infoButton.setTexture(this->_data->assets.GetTexture("Info Button"));
		...
		this->_infoButton.setPosition(this->_data->window.getSize().x - _infoButton.getGlobalBounds().width, this->_data->window.getSize().y-_infoButton.getGlobalBounds().height);
	...
	void MainMenuState::HandleInput()
	{
		...
		while (this->_data->window.pollEvent(event))
		{
			...
			{
				this->_data->machine.AddState(StateRef(new InterState(_data)), true);
			}

			if (this->_data->input.IsSpriteClicked(this->_infoButton, sf::Mouse::Left, this->_data->window))
			{
